...
import AddTasksDialog from '@/components/batches/tasks/AddTasksDialog.vue'
...
type TaskStatus = 'pending' | 'assigned' | 'in_progress' | 'completed' | 'under_review' | 'approved' | 'rejected' | 'draft'
interface Task {
    id: number
    status: TaskStatus
...
interface Props {
    project: Project
    batch: Batch
    // Your payload: tasks = { data: <paginator>, links: [...], meta: {...} }
    tasks: {
        data: TasksPaginator
        links: Array<{ url: string | null; label: string; active: boolean }>
        meta: {
    ...
    availableAudioFiles: Array<{ id: number; original_filename: string; duration: string; file_size: string }>
    can: { manage: boolean; publish: boolean; pause: boolean; resume: boolean; delete: boolean }
}
...
/* ===================== Computed ===================== */
// unwrap paginator from tasks.data
const paginator = computed<TasksPaginator>(() => props.tasks.data)

const listedTasks = computed<Task[]>(() => paginator.value?.data ?? [])

const pg = computed(() => ({
    current_page: paginator.value?.current_page ?? 1,
    last_page: paginator.value?.last_page ?? 1,
    per_page: paginator.value?.per_page ?? 25,
    total: paginator.value?.total ?? 0,
    from: paginator.value?.from ?? null,
    to: paginator.value?.to ?? null,
}))

/* ===================== Actions ===================== */
const publishBatch = () => {
    router.post(route('admin.projects.batches.publish', [props.project.id, props.batch.id]), {}, { preserveScroll: true })
}
const pauseBatch = () => {
    router.post(route('admin.projects.batches.pause', [props.project.id, props.batch.id]), {}, { preserveScroll: true })
}
const resumeBatch = () => {
    router.post(route('admin.projects.batches.resume', [props.project.id, props.batch.id]), {}, { preserveScroll: true })
}
const duplicateBatch = () => {
    router.post(route('admin.projects.batches.duplicate', [props.project.id, props.batch.id]), {}, {
        preserveScroll: true,
        onSuccess: () => (showActionsMenu.value = false),
    })
}
const editBatch = () => router.visit(route('admin.projects.batches.edit', [props.project.id, props.batch.id]))
...
const removeTask = (task: Task) => {
    if (!confirm('Remove this task from the batch?')) return
...
const onTasksAdded = () => {
    // Partial reload to refresh counts, tasks, and remaining available audios
    router.get(
        route('admin.projects.batches.show', [props.project.id, props.batch.id]),
        {},
        { preserveState: true, preserveScroll: true, replace: true, only: ['batch', 'tasks', 'availableAudioFiles'] },
    )
}
...
const handleClickOutside = (event: any) => {
    if (!event.target.closest('.actions-menu-container')) showActionsMenu.value = false
}
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
                    ...
                    Published {{ formatDate(batch.published_at) }}
                        ...
                        <div class="flex items-center gap-3" v-if="can.manage">
                            ...
                            <div class="relative actions-menu-container">
                                ...
                                >
                                    <div class="py-1">
                                        <button @click="duplicateBatch" class="flex items-center w-full px-4 py-2 text-sm hover:bg-muted">
            ...
            <Card>
                <CardHeader>
                    <CardTitle class="flex items-center gap-2">
                        <BarChart3 class="h-5 w-5" /> Progress Overview
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    ...
                    <Alert v-if="batch.total_tasks === 0" class="mt-4 border-amber-200 bg-amber-50">
                        ...
                        <AlertDescription class="text-amber-800">
                            ...
                            <p class="mt-1">Add audio files to create tasks before publishing this batch.</p>
                            <Button
                                v-if="batch.status === 'draft'"
                                @click="showAddTasksDialog = true"
                                size="sm"
                                variant="outline"
                ...
                <CardContent>
                    ...
                    <div v-else class="text-center py-16">
                        <div class="max-w-md mx-auto">
                            ...
                            <p class="text-muted-foreground mb-6">Add audio files to create tasks for annotation.</p>
                            <Button v-if="batch.status === 'draft' && can.manage" @click="showAddTasksDialog = true" class="gap-2">
                                <Plus class="h-4 w-4" /> Add Tasks
                            </Button>
                        </div>
        ...
        <AddTasksDialog
            ...
            @update:open="showAddTasksDialog = $event"
            :project-id="project.id"
            :batch-id="batch.id"
            :available-audio-files="availableAudioFiles"
            @added="onTasksAdded"
        />
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle class="flex items-center gap-2">
                        <AlertTriangle class="h-5 w-5 text-destructive" /> Delete Batch
                    </AlertDialogTitle>
                    <AlertDialogDescription>
